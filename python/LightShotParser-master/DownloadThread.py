...
class DownloadThread(threading.Thread):
    ...
    def run(self):
        def getTime(): return datetime.datetime.now().strftime(
            "%H:%M")  # функция для вывода текущего времени
        ...
        while self.__running:
            ...
            page_urls = self.__newUrls()
            print('It was checked in %f seconds' %
                  round(time.time() - duration, 3))
    ...
    def __getImage(self, page):  # функция для скачивания картинок
        # паттерн для поиска ссылки на картинку
        pattern = r"([\w\.-]+)(\.(jpg)|(png))"
        ...
        if image_name is not None:
            ...
            # print(image_name)
            badNames = ['footer-logo.png',
                        'icon_lightshot.png', '0_173a7b_211be8ff.png']
            if image_name not in badNames:
                image_url = 'https://image.prntscr.com/image/' + \
                    image_name  # создание ссылки на картинку
                # print(image_url)
