...
if len(sys.argv) != 1:  # проеврка заданы ли аргументы при запуске
    ...
    assert len(sys.argv) == 3, 'Wrong arguments: must be two arguments'
    assert sys.argv[1].isdigit() and sys.argv[2].isdigit(
    ), 'Wrong arguments: both arguments might be integer'  # проверка типа аргументов
...
print('Starting...')
downloadController = DC.DownloadController(
    countThreads, countPictures)  # создание контроллера потоков загрузки
